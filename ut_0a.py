...
from ut_mod_data import get_data, get_data_2 # Pobieramy sobie nasz moduł z funkcją którą chcemy testować

class DataTest(TestCase): #Tworzymy klase,k tóra dziedziczy po klasie TestCase z wczytanego modułu
    def test_first_last(self): # Definiujemy metody (czyli funkcje ale w klasie)
        data = get_data("Gary", "Feedbacker") # Aktywujemy sobie zmienna do któej przypisujemy aktywacje naszej testowanej funkcji
        self.assertEqual(data, 'Gary Feedbacker') # Używamy metody z zainportowanego modułu, do argumentów podajemy naszą zmienną i wzór do którego chcemy przyrównać wynik
